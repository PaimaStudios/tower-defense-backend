...
export function generateRandomMoves(
  ...
  const [gold, structures] = faction === "defender" 
  ...
  const toBuild = chooseStructures(matchConfig, matchState.mapState, faction, round, gold, structures)
  return toBuild
}
...
function chooseStructures(
  ...
  if (budget >= price) {
    const usableTileIndices = map.reduce((acc: number[], item, index) => (
      (item.type === "open" && item.faction === faction) ? [...acc, index]: acc
    ), []);
    const coordinates = Math.floor(Math.random() * usableTileIndices.length);
    const newMap: Tile[] = [...map.slice(0, coordinates), {type: "unbuildable", faction}, ...map.slice(coordinates + 1)]
    ...
    return chooseStructures(matchConfig, newMap, faction, round, budget - price, choices, [...chosen, action])
  }
  else return chosen
}