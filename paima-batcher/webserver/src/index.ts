...
import { AddressType, ErrorMessageFxn, keepRunning, setWebserverClosed, unsetWebserverClosed } from "@paima-batcher/utils";
...
async function initializeServer(errorCodeToMessage: ErrorMessageFxn, pool: Pool) {
    ...
    server.post("/submit_user_input", async (req, res) => {
        try {
            ...
            const requestValid = expectedProps.every(prop =>
            ...
            if (validity === 0) {
                try {
                    ...
                    console.log(
                        "[webserver] Error while setting input as validated."
                    );
                    res.status(500).json({
                        success: false,
