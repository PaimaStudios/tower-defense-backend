...
class BatchedTransactionPoster {
    ...
    constructor(
        ...
        this.storage = getStorageContract(
            this.web3,
    ...
    public updateWeb3 = (newWeb3: Web3) => {
        this.web3 = newWeb3;
    }
    ...
    private postMessage = async (msg: string): Promise<[number, string]> => {
        ...
        const tx = {
            data: this.storage.methods
                .paimaSubmitGameInput(hexMsg)
                .encodeABI(),
    ...
    private postingRound = async (): Promise<number> => {
        ...
        try {
            ...
            const [blockHeight, transactionHash] = await this.postMessage(
                batchedTransaction
            );
            ...
            console.log(
                "[batched-transaction-poster] Error while posting batched input:",
                err
            );
            return -1;
        }
    ...
    private buildBatchedTransaction = async (hashes: string[], ids: number[]): Promise<string> => {
        ...
        const validatedInputs = await getValidatedInputs.run(
            undefined,
            this.pool
        );
        ...
        for (let input of validatedInputs) {
            try {
                ...
                console.log(
                    "[batched-transaction-poster] Error while batching input:",
                    err
                );
            }
            if (!keepRunning) {
    ...
    private updatePostedStates = async (hashes: string[], blockHeight: number, transactionHash: string) => {
    ...
    private deletePostedInputs = async (ids: number[]) => {
        for (let id of ids) {
            try {
                await deleteValidatedInput.run({ id: id }, this.pool);
            } catch (err) {
                console.log(
                    "[batched-transaction-poster] Error while deleting processed inputs:",
                    err
                );
            }
        }
