...
import { AddressType, addressTypeName, ErrorCode, GenericRejectionCode, MAX_USER_INPUTS_PER_DAY, MAX_USER_INPUTS_PER_MINUTE } from "@paima-batcher/utils";
import {
...
class PaimaAddressValidator {
    ...
    private verifySignature = async (input: UserInput, addressType: AddressType): Promise<boolean> => {
        ...
        switch (addressType) {
            ...
            case AddressType.CARDANO:
                return verifySignatureCardano(
                    input.userAddress,
                    message,
                    input.userSignature
                );
            case AddressType.POLKADOT:
                return verifySignaturePolkadot(
                    input.userAddress,
                    message,
                    input.userSignature
                );
    ...
    private authenticateUserAddress = async (userAddress: string): Promise<boolean> => {
        ...
        try {
            ...
            if (result.length < 1) {
                ...
                await addUserTrackingEntry.run({ user_address: userAddress, current_timestamp: currentTimestamp }, this.pool);
            ...
            if (!isSameDay(latestTimestamp, currentTimestamp)) {
                if (this.evaluatePostingLimits(0, 0, userEntry.inputs_total)) {
                    await incrementUserTrackingAnotherDay.run({ user_address: userAddress, current_timestamp: currentTimestamp }, this.pool);
                    return true;
                }
                ...
                if (this.evaluatePostingLimits(0, userEntry.inputs_day, userEntry.inputs_total)) {
                    await incrementUserTrackingSameDay.run({ user_address: userAddress, current_timestamp: currentTimestamp }, this.pool);
                    return true;
                }
            } else {
                console.log(`[address-validator] ${userAddress}: same minute, updating entry`);
                if (this.evaluatePostingLimits(userEntry.inputs_minute, userEntry.inputs_day, userEntry.inputs_total)) {
                    await incrementUserTrackingSameMinute.run({ user_address: userAddress, current_timestamp: currentTimestamp }, this.pool);
                    return true;
                }
            }
            return false;
    ...
    private evaluatePostingLimits(inputsMinute: number, inputsDay: number, inputsTotal: number): boolean {
