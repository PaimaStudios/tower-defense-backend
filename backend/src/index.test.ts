import { getAllMaps, p } from "@tower-defense/db";
import { fillMap, getMap } from "@tower-defense/game-logic/src/map-processor";
import { PathTile, RawMap, Tile, TileNumber } from "@tower-defense/utils";
  test('all paths lead to other paths', async () => {
    const maps = await getAllMaps.run(undefined, p)
    const ok = maps.reduce((acc, mp) => {
      ...
      const paths: PathTile[] = fullMap.mapState.filter((t: Tile): t is PathTile => t.type === "path");
      const allArePaths = paths.reduce((acc, item) => {
        if (item.leadsTo.length < 1) return false
          ...
          const ok = item.leadsTo.reduce((acc, i) => {
            ...
            else if (fullMap.mapState[i].type === "base" && fullMap.mapState[i].faction === "defender" ) return acc
            else return false
          }, true)
          return ok ? acc : false
        }
