...
import {
    buildMatchExecutor,
    buildRoundExecutor,
} from "../helpers/executor-internals";
...
import {
    backendEndpointCall,
    indexerEndpointCall,
} from "../helpers/server-interaction";
import {
    AccountNftsData,
    FailedResult,
    LobbyState,
...
async function getLobbyState(
    ...
    try {
        ...
        if (l.lobby_state === "active") {
            if (
                !l.hasOwnProperty("round_start_height") ||
                !l.hasOwnProperty("round_length")
            ) {
                errorFxn(
...
async function getRoundExecutionState(
    ...
    try {
        ...
        if (
            !r.hasOwnProperty("roundStarted") ||
            !r.hasOwnProperty("roundLength")
        ) {
            return errorFxn(
                CatapultMiddlewareErrorCode.INVALID_RESPONSE_FROM_BACKEND
            );
...
async function getUserStats(
    ...
    try {
        ...
        if (
            ...
            userStats === null ||
            !userStats.hasOwnProperty("stats")
        ) {
            return errorFxn(
                CatapultMiddlewareErrorCode.INVALID_RESPONSE_FROM_BACKEND,
        ...
        for (let prop of ["wallet", "wins", "losses", "ties"]) {
...
async function getUserWalletNfts(
    ...
    try {
        ...
        let resultList = response.result;
