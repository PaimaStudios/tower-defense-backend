...
export async function submitToBatcher(subunit: BatchedSubunit): Promise<Result<number>> {
  ...
  try {
    if (!response.success) {
      ...
      return errorFxn(CatapultMiddlewareErrorCode.BATCHER_REJECTED_INPUT, msg);
    }
    hash = response.hash;
  } catch (err) {
    return errorFxn(CatapultMiddlewareErrorCode.INVALID_RESPONSE_FROM_BATCHER, err);
  }
  ...
  try {
    for (let i = 0; i < BATCHER_RETRIES; i++) {
      ...
      const result = await batcherEndpointCall(`track_user_input?input_hash=${hash}`);
      if (!result.success) {
        continue;
      }
      // TODO: verify?
      const status: BatcherTrackResponse = result.result as BatcherTrackResponse;
    ...
    return errorFxn(CatapultMiddlewareErrorCode.FAILURE_VERIFYING_BATCHER_ACCEPTANCE);
  } catch (err) {
    return errorFxn(CatapultMiddlewareErrorCode.FAILURE_VERIFYING_BATCHER_ACCEPTANCE, err);
  }
}
