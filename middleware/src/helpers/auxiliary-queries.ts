...
import { backendEndpointCall, backendTextEndpointCall } from './server-interaction';
...
> {
  ...
  try {
    ...
    if (typeof j !== 'object' || j === null || !j.hasOwnProperty('block_height')) {
      return errorFxn(CatapultMiddlewareErrorCode.INVALID_RESPONSE_FROM_BACKEND);
    }
    return {
      success: true,
...
export async function getRawNewLobbies(
  ...
  try {
    ...
    if (typeof j !== 'object' || j === null || !j.hasOwnProperty('lobbies')) {
      return errorFxn(CatapultMiddlewareErrorCode.INVALID_RESPONSE_FROM_BACKEND);
    }
...
export async function getRemoteBackendVersion(): Promise<string> {
  ...
  if (!result.success) {
    throw new Error('Error while querying version string: ' + result.message);
  }
