...
export async function postConciselyEncodedData(
  ...
  switch (postingMode) {
    case PostingMode.UNBATCHED:
      return postString(sendMetamaskWalletTransaction, userAddress, methodName, gameInput);
...
export async function postString(
  ...
  const tx = buildDirectTx(userAddress, methodName, dataUtf8);
...
async function verifyBatcherSubmission(inputHash: string): Promise<Result<number>> {
  ...
  for (let i = 0; i < BATCHER_RETRIES; i++) {
    ...
    try {
      ...
      errorFxn(CatapultMiddlewareErrorCode.ERROR_QUERYING_BATCHER_ENDPOINT, err);
      continue;
    }
    ...
    try {
      ...
      errorFxn(CatapultMiddlewareErrorCode.INVALID_RESPONSE_FROM_BATCHER, err);
      continue;
    }
  }
