...
import { MatchExecutor as MatchExecutorConstructor, RoundExecutor as RoundExecutorConstructor, } from "paima-engine/paima-executors";
import Prando from "paima-engine/paima-prando";
// executor
export async function buildRoundExecutor(data) {
    const { seed } = data.block_height;
    const matchEnvironment = { something: "something" };
    const matchState = data.state;
    const rng = new Prando(seed);
    return RoundExecutorConstructor.initialize(matchEnvironment, matchState, data.moves, rng, processTick);
}
// TODO a lot
export async function buildMatchExecutor(data) {
    const { config, states, seeds, moves } = data;
    const matchEnvironment = { something: "something" };
    console.log(seeds, "seeds used for the match executor at the middleware");
    const stateMutator = (m) => m[0];
    return MatchExecutorConstructor.initialize(matchEnvironment, config.num_of_rounds, states, seeds, moves, stateMutator, processTick);
}
function randomizeOrder(player1moves, player2moves, randomnessGenerator) {
...
function sortRoundMoves(moves, randomnessGenerator) {
